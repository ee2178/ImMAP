...
class GroupCDL(BaseUnrolledModel):
    def __init__(self, M=169, Mh=64, C=1, P=7, sc=2, K=30, W=35, dK=5,
                 ...
                 attn_backend="gather", blend=True, flex_block_size=128, is_complex= True, fenchel = False, init = True):
        super().__init__()
    ...
    def forward(self, y, sigma=0.0, E = None):
        ...
        for k in range(self.K):
            ...
            if self.fenchel is False:
                # GroupCDL
                z = self._threshold(z - v, apply_adj, tau_map)
            elif self.fenchel is True:
                # GroupLPDS (an application of Moreau's Identity!)
                # i.e. group clipping
                z = z - self._threshold(z - v, apply_adj, tau_map)
