...
# ---------------------------------------------------------------------------
# ImMAP-SB: self-paced annealed Tweedie ascent (ImMAP counterpart to i2sb_sample)
# ---------------------------------------------------------------------------
@torch.no_grad()
def immap_sb(net, y, sched, cond=None, tau=0.19, beta=0.05, t_L=0.01, h_0=0.01,
             ...
             parameterization="x0", target_channels=1,
             max_iter=1000, log_every=None, verbose=True):
    """ImMAP-SB: anneal toward argmax_x log p(x | y) with an I2SB regressor
    r(x_t, t, y) ~ E[x0 | x_t, y]. ImMAP-style counterpart to i2sb_sample -- instead of a fixed
    discrete reverse schedule, the bridge level t is SELF-ESTIMATED each step and the loop runs
    until t < t_L (cf. immap2's self-estimated sigma_t in diffusion/immap.py).
    ...
    Geometry is the t(1-t) (Brownian) bridge, so this is most consistent with a net trained on
    bridge_type="brownian", shape="constant". A net trained on the symmetric "i2sb" schedule still
    works (the regressor is just a denoiser), with a mild geometric mismatch in the annealing.

    `sched` is used only to map the continuous t to the sigma the net was conditioned on (nearest
    discrete step) and, for parameterization="eps", to recover x0. Pass the SAME schedule the
    ...
    * eps ~ CN(0, I) in the algorithm; for the real-valued pipeline randn_like gives real N(0, I).
    ...
    if warm_start:
        ...
        # interpolant at init_t. Then the self-estimated t is ~init_t (< 1), not 1 -- so the loop
        # never sees x=y and can't get trapped. Requires the first pass to be decent.
        step1 = torch.full((x.shape[0],), N - 1, device=device, dtype=torch.long)   # t = 1 endpoint
        sig1 = get_std_fwd(sched, step1, xdim=y.shape[1:])
        out1 = cdlnet_pred(net, y, sig1, cond=cond, target_channels=target_channels)
        x0_init = out1 if parameterization == "x0" else compute_pred_x0(sched, step1, y, out1)
        tb0 = unsqueeze_xdim(t, y.shape[1:])
        x = (1.0 - tb0) * x0_init + tb0 * y                       # x <- bridge mean at init_t
        if warm_start_noise:
            step_it = (t.clamp(0.0, 1.0) * (N - 1)).round().long()
            x = x + unsqueeze_xdim(sched.std_sb[step_it], y.shape[1:]) * torch.randn_like(x)
    k = 1
    ts, xs = [], []
    ...
    while float(t.max()) > t_L and k <= max_iter:
        ...
        sigma = get_std_fwd(sched, step, xdim=x.shape[1:])
        out = cdlnet_pred(net, x, sigma, cond=cond, target_channels=target_channels)
        x_hat0 = out if parameterization == "x0" else compute_pred_x0(sched, step, x, out)
